    ...
    function tryActivate(row, col) {
        if (count) {
            ...
            currentIndex = Math.min(
                row ? ((Math.max(1, row) * columns) + col) : col, count - 1
            );
    ...
    DropArea {
        ...
        onPositionChanged: event => {
            ...
            if (item) {
                ...
                itemGrid.model.dropPlaceholderIndex = (
                    hasPlaceholder ? itemGrid.model.count - 1 : itemGrid.model.count
                );
        ...
        onDropped: {
            if (
                kicker.dragSource && kicker.dragSource.parent !== gridView.contentItem
                && kicker.dragSource.GridView.view.model.favoritesModel === itemGrid.model
            ) {
                itemGrid.model.addFavorite(
                    kicker.dragSource.favoriteId,
                    itemGrid.model.dropPlaceholderIndex
                );
                gridView.currentIndex = -1;
            }
        ...
        PlasmaComponents.ScrollView {
            ...
            GridView {
                ...
                highlight: Item {
                    property bool isDropPlaceHolder: (
                        "dropPlaceholderIndex" in itemGrid.model
                        && itemGrid.currentIndex === itemGrid.model.dropPlaceholderIndex
                    )
                ...
                Keys.onDownPressed: event => {
                    if (itemGrid.currentRow() < itemGrid.lastRow()) {
                        // Fix moveCurrentIndexDown()'s lack of proper
                        // spatial nav down into partial columns.
                        event.accepted = true;
                        var columns = Math.floor(width / cellWidth);
        ...
        MouseArea {
            ...
            onPositionChanged: (mouse) => {
                ...
                if (gridView.currentIndex !== -1) {
                    if (
                        itemGrid.dragEnabled && pressX !== -1
                        && dragHelper.isDrag(pressX, pressY, mouse.x, mouse.y)
                    ) {
                        if ("pluginName" in item.m) {
                            dragHelper.startDrag(
                                kicker, item.url, item.icon, "text/x-plasmoidservicename", item.m.pluginName
                            );
                        } else {
